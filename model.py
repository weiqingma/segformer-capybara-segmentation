...
class OverlapPatchEmbed(nn.Module):
    #输入x：[B, 3, H, W]
    #输出x: [B, H'*W', embed_dim], H' = H/4, W' = W/4
...
#实现SegFormer中的Efficient Self-Attention
#Query使用完整token， Key和Value通过sr_ratio做空间降采样以降低计算量
#Q.shape = [B, N, C]
#k.shape = v.shape = [B, num_heads, N, head_dim]
class Attention(nn.Module):
    def __init__(self, dim, num_heads = 8, qkv_bias = False, qk_scale = None, attn_drop = 0.5, proj_drop = 0., sr_ratio = 1 ):
        super().__init__()
...
class DWConv(nn.Module):
    def __init__(self, dim = 768):
        ...
        #groups = dim, 每个通道自己做卷积，提取空间特征，不和其他通道混合；同时更轻量
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4, qkv_bias = False, qk_scale = None, drop = 0. , attn_drop = 0., act_layer = nn.GELU(), norm_layer = nn.LayerNorm, sr_ratio = 1):
        ...
        self.mlp = Mlp_backbone(in_features = dim, hidden_features=int(dim*mlp_ratio), act_layer = act_layer, drop = drop)
...
class MixVisionTransformer(nn.Module):
    def __init__(self, in_chans = 3, num_classes = 21, embed_dims = [32, 64, 160, 256], 
...
class mit_b0(MixVisionTransformer):
    def __init__(self, pretrained = False):
        super().__init__(embed_dims = [32, 64, 160, 256], num_heads = [1, 2, 5, 8], mlp_ratios = [4, 4, 4, 4],
...
class SegFormer(nn.Module):
    def __init__(self, num_classes = 21, in_channels = [32, 64, 160, 256], embedding_dim = 256, pretrained = False):
        super().__init__()
